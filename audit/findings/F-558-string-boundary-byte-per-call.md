...
Recommendation: implementation change. Move bytes in bulk, for example through an exported memory buffer or a length-prefixed copy, which is also what the Component Model canonical ABI requires for strings. Decided rule (spec/10-modules.md, Wasm Boundary): the Component Model with canonical-ABI strings is the only normative host boundary.
