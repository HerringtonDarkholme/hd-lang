;; commit bd985d7; command: om-build.ts (emitWat of audit/probes/arch/object-model/om-generic-data.hd); date: 2026-09-25T20:54:06.500Z
;; excerpt: rec types, user globals, user functions (runtime $hd.* functions omitted)
(rec
    (type $sig0 (func (param anyref) (param i32) (result (ref null $d0))))
...
(func $f0 (param $l0 (ref null $d0)) (result anyref)
    (struct.get $d0 $d0f0 (local.get $l0))
  )

(func $f1 (param $l0 i32) (result (ref null $d0))
    (local $tmp0 i32)
    (block (result (ref null $d0))
      (local.set $tmp0 (local.get $l0))
      (struct.new $d0 (struct.new $hd.box-i32 (local.get $tmp0)))
    )
  )

(func $fv1 (type $sig0) (param $env anyref) (param $l0 i32) (result (ref null $d0))
...
(func $f2 (export "main") (result i32)
    ...
    (local $l4 i32)
    (local $tmp0 f64)
    (local $tmp1 (ref null $hd.bytes))
    (local.set $l0 (call $f1 (i32.const 40)))
    (local.set $l1 (block (result (ref null $d0))
      (local.set $tmp0 (f64.const 2.5))
      (struct.new $d0 (struct.new $hd.box-f64 (local.get $tmp0)))
    ))
    (local.set $l2 (block (result (ref null $d0))
      (local.set $tmp1 (array.new_fixed $hd.bytes 2 (i32.const 97) (i32.const 98)))
      (struct.new $d0 (local.get $tmp1))
    ))
    (local.set $l3 (struct.get $hd.box-i32 $hd.box-i32-value (ref.cast (ref $hd.box-i32) (struct.get $d0 $d0f0 (local.get $l0)))))
    (local.set $l4 (struct.get $hd.box-i32 $hd.box-i32-value (ref.cast (ref $hd.box-i32) (call $f0 (local.get $l0)))))
    (call $hd.add_i32 (call $hd.add_i32 (call $hd.sub_i32 (call $hd.add_i32 (local.get $l3) (local.get $l4)) (i32.const 40)) (call $hd.string_len (ref.cast (ref null $hd.bytes) (struct.get $d0 $d0f0 (local.get $l2))))) (call $hd.sub_i32 (i32.const 1) (call $f3 (struct.get $hd.box-f64 $hd.box-f64-value (ref.cast (ref $hd.box-f64) (struct.get $d0 $d0f0 (local.get $l1)))) (f64.const 2))))
  )

(func $fv2 (type $sig1) (param $env anyref)  (result i32)
    (call $f2)
  )
...
(func $fv3 (type $sig2) (param $env anyref) (param $l0 f64) (param $l1 f64) (result i32)
