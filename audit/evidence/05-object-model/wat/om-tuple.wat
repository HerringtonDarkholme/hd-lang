;; commit bd985d7; command: om-build.ts (emitWat of audit/probes/arch/object-model/om-tuple.hd); date: 2026-09-25T20:54:06.500Z
;; excerpt: rec types, user globals, user functions (runtime $hd.* functions omitted)
(rec
    (type $sig0 (func (param anyref) (param i32) (param f64) (result (ref null $hd.list))))
    (type $sig1 (func (param anyref) (param (ref null $hd.list)) (result i32)))
...
(func $f0 (param $l0 i32) (param $l1 f64) (result (ref null $hd.list))
    (array.new_fixed $hd.list 3 (struct.new $hd.box-i32 (local.get $l0)) (struct.new $hd.box-f64 (local.get $l1)) (array.new_fixed $hd.bytes 1 (i32.const 115)))
  )

(func $fv0 (type $sig0) (param $env anyref) (param $l0 i32) (param $l1 f64) (result (ref null $hd.list))
    (call $f0 (local.get $l0) (local.get $l1))
  )

(func $f1 (param $l0 (ref null $hd.list)) (result i32)
    (struct.get $hd.box-i32 $hd.box-i32-value (ref.cast (ref $hd.box-i32) (array.get $hd.list (ref.as_non_null (local.get $l0)) (i32.const 0))))
  )

(func $fv1 (type $sig1) (param $env anyref) (param $l0 (ref null $hd.list)) (result i32)
    (call $f1 (local.get $l0))
  )

(func $f2 (export "main") (result i32)
    (local $l0 (ref null $hd.list))
    (local $l1 (ref null $hd.list))
    (local $l2 i32)
    (local $l3 f64)
    (local $l4 (ref null $hd.bytes))
    (local.set $l0 (call $f0 (i32.const 40) (f64.const 1.5)))
    (local.set $l1 (local.get $l0))
    (local.set $l2 (struct.get $hd.box-i32 $hd.box-i32-value (ref.cast (ref $hd.box-i32) (array.get $hd.list (ref.as_non_null (local.get $l1)) (i32.const 0)))))
    (local.set $l3 (struct.get $hd.box-f64 $hd.box-f64-value (ref.cast (ref $hd.box-f64) (array.get $hd.list (ref.as_non_null (local.get $l1)) (i32.const 1)))))
    (local.set $l4 (ref.cast (ref null $hd.bytes) (array.get $hd.list (ref.as_non_null (local.get $l1)) (i32.const 2))))
    (call $hd.sub_i32 (call $hd.add_i32 (call $hd.add_i32 (call $hd.add_i32 (call $f1 (local.get $l0)) (call $hd.string_len (ref.cast (ref null $hd.bytes) (array.get $hd.list (ref.as_non_null (local.get $l0)) (i32.const 2))))) (call $f3 (local.get $l3) (f64.const 1))) (local.get $l2)) (i32.const 40))
  )
...
(func $f3 (export "above") (param $l0 f64) (param $l1 f64) (result i32)
...
(func $fv3 (type $sig3) (param $env anyref) (param $l0 f64) (param $l1 f64) (result i32)
    (call $f3 (local.get $l0) (local.get $l1))
  )
