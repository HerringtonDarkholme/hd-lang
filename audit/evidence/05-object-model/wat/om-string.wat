;; commit bd985d7; command: om-build.ts (emitWat of audit/probes/arch/object-model/om-string.hd); date: 2026-09-25T20:54:06.500Z
;; excerpt: rec types, user globals, user functions (runtime $hd.* functions omitted)
(rec
    (type $sig0 (func (param anyref) (param (ref null $hd.bytes)) (result i32)))
...
(func $f0 (param $l0 (ref null $hd.bytes)) (result i32)
    (call $hd.string_len (local.get $l0))
  )

(func $fv0 (type $sig0) (param $env anyref) (param $l0 (ref null $hd.bytes)) (result i32)
    (call $f0 (local.get $l0))
  )

(func $f1 (export "concat_loop") (param $l0 i32) (result i32)
    (local $l1 (ref null $hd.bytes))
    (local $l2 i32)
    (local $tmp0 i32)
    (local.set $l1 (array.new_default $hd.bytes (i32.const 0)))
    ...
    (block $break0
      (loop $loop0
        ...
        (local.set $l1 (call $hd.string_concat (local.get $l1) (array.new_fixed $hd.bytes 1 (i32.const 120))))
        (local.set $l2 (call $hd.add_i32 (local.get $l2) (i32.const 1)))
        (br $loop0)
      )
    )
    (call $hd.string_len (local.get $l1))
  )
...
(func $f2 (export "interp_loop") (param $l0 i32) (result i32)
    (local $l1 (ref null $hd.bytes))
    (local $l2 i32)
    (local $tmp0 i32)
    (local.set $l1 (array.new_default $hd.bytes (i32.const 0)))
...
(func $f3 (export "interp_fresh") (param $l0 i32) (result i32)
    (local $l1 i32)
    (local $l2 i32)
    ...
    (block $break2
      (loop $loop2
        ...
        (local.set $l3 (call $hd.string_concat (call $hd.string_concat (array.new_fixed $hd.bytes 2 (i32.const 118) (i32.const 61)) (call $hd.i32_to_string (local.get $l2))) (array.new_fixed $hd.bytes 1 (i32.const 59))))
...
(func $fv3 (type $sig1) (param $env anyref) (param $l0 i32) (result i32)
    (call $f3 (local.get $l0))
  )

(func $f4 (export "main") (result i32)
    (call $hd.add_i32 (call $hd.add_i32 (call $hd.add_i32 (call $f1 (i32.const 3)) (call $f2 (i32.const 3))) (call $f0 (array.new_fixed $hd.bytes 4 (i32.const 231) (i32.const 149) (i32.const 140) (i32.const 120)))) (call $f3 (i32.const 2)))
  )
