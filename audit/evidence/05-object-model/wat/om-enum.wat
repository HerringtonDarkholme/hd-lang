;; commit bd985d7; command: om-build.ts (emitWat of audit/probes/arch/object-model/om-enum.hd); date: 2026-09-25T20:54:06.500Z
;; excerpt: rec types, user globals, user functions (runtime $hd.* functions omitted)
(rec
    (type $sig0 (func (param anyref) (param (ref null $e0)) (result i32)))
    (type $sig1 (func (param anyref) (result i32)))
    (type $sig2 (func (param anyref) (param f64) (param f64) (result i32)))
...
(global $e0v3 (ref $e0)
    (struct.new $e0 (i32.const 3) (i32.const 0) (i32.const 0) (i32.const 0) (f64.const 0)))

(global $e1v0 (ref $e1)
    (struct.new $e1 (i32.const 0)))

(global $e1v1 (ref $e1)
    (struct.new $e1 (i32.const 1)))

(global $e1v2 (ref $e1)
    (struct.new $e1 (i32.const 2)))

(func $f0 (param $l0 (ref null $e0)) (result i32)
    (local $l1 i32)
    (local $l2 i32)
    (local $l3 i32)
    (local $l4 f64)
    ...
    (block (result i32)
      ...
      (if (result i32)
        ...
        (then
          (local.set $l1 (struct.get $e0 $e0f0 (local.get $tmp0)))
          (call $hd.mul_i32 (call $hd.mul_i32 (i32.const 3) (local.get $l1)) (local.get $l1))
        )
        (else
          (if (result i32)
            (i32.eq (struct.get $e0 $e0tag (local.get $tmp0)) (i32.const 1))
            (then
              (local.set $l2 (struct.get $e0 $e0f1 (local.get $tmp0)))
              (local.set $l3 (struct.get $e0 $e0f2 (local.get $tmp0)))
              (call $hd.mul_i32 (local.get $l2) (local.get $l3))
            )
            (else
              (if (result i32)
                (i32.eq (struct.get $e0 $e0tag (local.get $tmp0)) (i32.const 2))
                (then
                  (local.set $l4 (struct.get $e0 $e0f3 (local.get $tmp0)))
                  (call $f2 (local.get $l4) (f64.const 0))
                )
                (else
                  (if (result i32)
                    (i32.eq (struct.get $e0 $e0tag (local.get $tmp0)) (i32.const 3))
                    (then
                      (i32.const 0)
                    )
                    (else
...
(func $fv0 (type $sig0) (param $env anyref) (param $l0 (ref null $e0)) (result i32)
...
(func $f1 (export "main") (result i32)
    ...
    (local $tmp2 i32)
    (local $tmp3 f64)
    (call $hd.sub_i32 (call $hd.add_i32 (call $hd.add_i32 (call $f0 (block (result (ref null $e0))
      (local.set $tmp0 (i32.const 2))
      (struct.new $e0 (i32.const 0) (local.get $tmp0) (i32.const 0) (i32.const 0) (f64.const 0))
    )) (call $f0 (block (result (ref null $e0))
      (local.set $tmp1 (i32.const 3))
      (local.set $tmp2 (i32.const 10))
      (struct.new $e0 (i32.const 1) (i32.const 0) (local.get $tmp1) (local.get $tmp2) (f64.const 0))
    ))) (call $f0 (global.get $e0v3))) (call $f0 (block (result (ref null $e0))
      (local.set $tmp3 (f64.const 1))
      (struct.new $e0 (i32.const 2) (i32.const 0) (i32.const 0) (i32.const 0) (local.get $tmp3))
    )))
  )
...
(func $f2 (export "above") (param $l0 f64) (param $l1 f64) (result i32)
...
(func $fv2 (type $sig2) (param $env anyref) (param $l0 f64) (param $l1 f64) (result i32)
