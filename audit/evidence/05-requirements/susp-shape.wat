;; commit bd985d7; command: hd build --wat audit/probes/arch/suspension/shape.hd; date: 2026-09-25T20:50:10Z
(module
...
(func $hd.suspension_drive (param $frame (ref null $hd.suspension)) (result anyref)
  ...
  (func $f1 (result (ref null $s1))
    ...
    (struct.new $s1 (i32.const 0) (i32.const 0) (i32.const 0) (i32.const 0) (i32.const 0) (ref.null $s0) (ref.null $s0) (i32.const 0))
  )
  ...
  (func $poll1 (param $frame (ref null $s1)) (result i32)
    ...
    (if (i32.le_u (local.get $resume-state) (i32.const 4))
      (then
        (local.set $l0 (i32.const 10))
        (struct.set $s1 $s1child0 (local.get $frame) (call $f0 (local.get $l0)))
        (if (i32.eqz (call $poll0 (struct.get $s1 $s1child0 (local.get $frame))))
          (then
            ...
            (call $hd.trace (i32.const 1) (i32.const 6))
            (return (i32.const 0))))
        (local.set $l1 (struct.get $s0 $s0result (struct.get $s1 $s1child0 (local.get $frame))))
        (struct.set $s1 $s1child1 (local.get $frame) (call $f0 (local.get $l1)))
        (if (i32.eqz (call $poll0 (struct.get $s1 $s1child1 (local.get $frame))))
          (then
            ...
            (call $hd.trace (i32.const 1) (i32.const 6))
            (return (i32.const 0))))
        (local.set $l2 (struct.get $s0 $s0result (struct.get $s1 $s1child1 (local.get $frame))))
        (struct.set $s1 $s1result (local.get $frame) (call $hd.add_i32 (call $hd.add_i32 (local.get $l0) (local.get $l1)) (local.get $l2)))
