;; commit bd985d7; emitted by audit/scripts/arch/req-probes.ts for row-chain-1.hd; 2026-09-25T20:54:25.421Z
(module
  (import "hd" "panic" (func $hd.panic (param i32)))
  (rec
    (type $sig0 (func (param anyref) (param i32) (param (ref null $trait0)) (result i32)))
...
(func $hd.suspension_drive (param $frame (ref null $hd.suspension)) (result anyref)
  ...
  (elem declare func $fv0 $fv1 $fv2 $fv3 $fv4 $fv5 $fv6 $fv7 $fv8 $fv9 $fv10 $fv11 $fv12 $adapt0 $tadapt0_0)

  (func $f0 (export "leaf") (param $l0 i32) (param $provider0 (ref null $trait0)) (result i32)
  ...
  (func $fv0 (type $sig0) (param $env anyref) (param $l0 i32) (param $provider0 (ref null $trait0)) (result i32)
    (call $f0 (local.get $l0) (local.get $provider0))
  )
  ...
  (func $f11 (export "main") (result i32)
    ...
    (block (result i32)
      ...
      (block $break0
        (loop $loop0
          (br_if $break0 (i32.eqz (block (result i32) (local.set $tmp6 (if (result i32) (i32.lt_s (local.get $l1) (i32.const 200000)) (then (i32.const -1)) (else (if (result i32) (i32.gt_s (local.get $l1) (i32.const 200000)) (then (i32.const 1)) (else (i32.const 0)))))) (i32.eq (local.get $tmp6) (i32.const -1)))))
          (local.set $l2 (block (result i32)
            (local.set $tmp4 (call $hd.add_i32 (local.get $l2) (call $f10 (struct.new $closure1 (ref.func $adapt0) (struct.new $closure0 (ref.func $fv0) (ref.null any))) (block (result i32)
            (local.set $tmp2 (local.get $l1))
            (local.set $tmp3 (i32.const 7))
            (if (i32.eqz (local.get $tmp3))
              (then (call $hd.panic (i32.const 1)) unreachable))
            (i32.rem_s (local.get $tmp2) (local.get $tmp3))
          ) (struct.new $hd.providers (i32.const 0) (local.get $l0) (ref.null $hd.providers)))))
            (local.set $tmp5 (i32.const 1000003))
            (if (i32.eqz (local.get $tmp5))
              (then (call $hd.panic (i32.const 1)) unreachable))
            (i32.rem_s (local.get $tmp4) (local.get $tmp5))
          ))
  ...
  (func $adapt0 (type $sig1) (param $env anyref) (param $a0 i32) (param $p0 (ref null $hd.providers)) (result i32)
    (call_ref $sig0 (struct.get $closure0 $closure0env (ref.cast (ref $closure0) (local.get $env))) (local.get $a0) (ref.cast (ref null $trait0) (call $hd.provider_get (call $hd.provider_concat (local.get $p0) (ref.null $hd.providers)) (i32.const 0))) (struct.get $closure0 $closure0fn (ref.cast (ref $closure0) (local.get $env))))
  )
