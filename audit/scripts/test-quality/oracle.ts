// Oracle-strength checks for reject, warn, and panic cases (audit step 1.6).
// For each marked case: (a) neutralize the marked line in a copy under
// audit/probes/oracle/ and rerun the real compiler; (b) record every other
// diagnostic the original produces. Writes oracle-runs.tsv.
// Usage: node --experimental-strip-types audit/scripts/test-quality/oracle.ts [--jobs N]
...
const probeRoot = resolve(root, "audit/probes/oracle");
...
async function main(): Promise<void> {
  ...
  const cases = [...readConformance(), ...readFixtures()].filter((fixture) => fixture.markers.length > 0);
  ...
  writeFileSync(
    resolve(out, "oracle-runs.tsv"),
    `${header("node --experimental-strip-types audit/scripts/test-quality/oracle.ts")}\n${tsv([head, ...rows])}\n`,
  );
