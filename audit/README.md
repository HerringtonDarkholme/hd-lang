...
| L10 | A default naming a later parameter is `binding-not-yet-visible`. |
| R1 | Defaults, annotation builders, and non-entry module initialization are requirement-free: no provider use and no suspension, checked from callee signatures only. A provider use is `requirement-in-default`. |
