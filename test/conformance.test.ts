...
const TYPE_CASES = [
  ...
  ["typing/valid/generic-data-embedding.hd", "accept"],
  ...
  ["typing/valid/module-bindings.hd", "accept"],
  ["typing/valid/transitive-initialized-binding.hd", "accept"],
  ["typing/invalid/forward-module-binding.hd", "reject:binding-not-yet-visible"],
  [
    "typing/invalid/transitive-uninitialized-binding.hd",
    "reject:top-level-read-before-initialization",
  ],
  ...
  [
    ...
    "reject:recursive-closure-needs-result-type",
  ],
  ...
  [
    ...
    "reject:generic-requirement-key-collision",
  ],
...
test("curated chapter-01 and chapter-02 conformance cases match the manifest", async () => {
  ...
  for (const [path, expectation] of CASES) {
    assert.ok(
      manifest.includes(`${path}\tparse\t${expectation}\t`),
      `${path} must retain its manifest expectation`,
    );
...
test("curated type conformance cases match the manifest", async () => {
  ...
  for (const [path, expectation] of TYPE_CASES) {
    assert.ok(
      manifest.includes(`${path}\ttype\t${expectation}\t`),
      `${path} must retain its manifest expectation`,
    );
    const source = await readFile(resolve("spec/conformance", path), "utf8");
    ...
    if (expectation === "accept") {
      ...
      assert.ok(
        result.diagnostics.some(
          (diagnostic) => diagnostic.code === code && diagnostic.severity === "warning",
        ),
        `${path} should warn ${code}`,
      );
    }
  }
});
...
test("curated runtime panic cases match the manifest and trap in Wasm", async () => {
  ...
  for (const [path, expectation] of RUNTIME_CASES) {
    assert.ok(
      manifest.includes(`${path}\truntime\t${expectation}\t`),
      `${path} must retain its manifest expectation`,
    );
    const source = await readFile(resolve("spec/conformance", path), "utf8");
...
test("curated runtime-valid cases compile and complete in Wasm", async () => {
  ...
  for (const path of RUNTIME_VALID_CASES) {
    assert.ok(
      manifest.includes(`${path}\truntime\taccept\t`),
      `${path} must retain its manifest expectation`,
    );
    const source = await readFile(resolve("spec/conformance", path), "utf8");
    const { instance } = await instantiate(source);
